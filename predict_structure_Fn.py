...
class FindPhotovoltaicMaterial(PredictStructure):
    ...
    def get_structure_properties(self, struc_param: dict, **kwargs):
        ...
        try:
            ...
            target_property = self.score_fun(formation_energy, Eg_moo)
            # target_property = self.score_fun(formation_energy, Eg_moo)

            save_target = [target_property, formation_energy, formation_energy_moo, Eg, Eg_moo]
    ...
    @staticmethod
    def score_fun(*x):
        formation_energy_, Eg_ = x
        score = formation_energy_/5.0 + Eg_
        if score < 0:
            score = np.exp(score)
        else:
            score += 1
        # print(score, formation_energy_, Eg_)
        return score

    def create_save_results_data_file(self):
        with open(self.step_result_file, 'w+') as f:
            f.write("number,step,compound,sg_number,child_cell,score,formation_energy_moo,band_gap_moo,formation_energy,band_gap,time\n")
    ...
    def save_data_for_successful_step(self, target_property, struc_param: dict, struc: Structure, expand_param, **kwargs):
        ...
        target_property, formation_energy, formation_energy_moo, Eg, Eg_moo = target_property
        ...
        structure_file_name = os.path.join(
            ...
            '%f_%f_%f_%s_%d_%d.cif' % (target_property, formation_energy, Eg, formula, structure_number, step_number))
    ...
    def Eg_process_2(self, _Eg, **kwargs):
        biases_range = [0.1, 1.0, 2.5]
        biases_scale = [999.0, 1.0, 20]
