...
if __name__ == '__main__':
    ...
    # ----------------------------------------------
    # elements_used_count = {k:len(v) for k,v in elements_energy_list.items()}
    # ele_comb_list = []
    # while all_elements:
    #     random_items = random.choices(all_elements, k=composition_count)
    #     for i in set(random_items):
    ...
    save_data_csv(output_path, 'ele_prob.csv',
                  ...
                  header=['elements', 'mu', 'sigma', 'prob(> 250)', 'target'])
    ...
    element_comb_energy_list_top = sorted(element_comb_energy_list.items(), key=lambda x: max(x[1]), reverse=True)
    ...
    for i, (k, v) in enumerate(element_comb_energy_list_top):
        ...
        ic = get_config(_eles='[%d] [%d]' % (Element(ele[0]).Z, Element(ele[1]).Z),
                        ...
                        _max_step='10000',
                        _wyck_pos_gen='3',
                        _space_group='[16-74,89-142,177-194,207-230]')
        config_list.append(ic)
