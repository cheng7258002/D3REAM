...
class FindBulkModulusMaterial(PredictStructure):
    ...
    def get_structure_properties(self, struc_param: dict, **kwargs):
        ...
        try:
            ...
            target_property = self.score_fun(formation_energy, BM)
            # target_property = self.score_fun(formation_energy, Eg_moo)

            save_target = [target_property, formation_energy, BM]
    ...
    @staticmethod
    def score_fun(*x):
        formation_energy_, BM_ = x
        score = formation_energy_/5.0 - BM_
        if score < 0:
            score = np.exp(score)
        else:
            score += 1
        # print(score, formation_energy_, BM_)
        return score

    def create_save_results_data_file(self):
        with open(self.step_result_file, 'w+') as f:
            f.write("number,step,compound,sg_number,child_cell,score,formation_energy,bulk_moduli,time\n")
    ...
    def save_data_for_successful_step(self, target_property, struc_param: dict, struc: Structure, expand_param, **kwargs):
        ...
        target_property, formation_energy, BM = target_property
        ...
        structure_file_name = os.path.join(
            ...
            '%f_%f_%f_%s_%d_%d.cif' % (target_property, formation_energy, BM, formula, structure_number, step_number))
